...
class JobSeekerDashboardWeb extends StatefulWidget {
  const JobSeekerDashboardWeb({super.key});
  @override
...
class _JobSeekerDashboardWebState extends State<JobSeekerDashboardWeb> {
  ...
  void _calculateStats() {
    ...
    selectedCount = jobController.jobSeekersAppliedLists
        .where((e) => e.status == "Selected")
        .length;
    rejectedCount = jobController.jobSeekersAppliedLists
                      ...
                      if (!isDesktop)
                        Positioned(
                          ...
                          child: IconButton(
                            ...
                            onPressed: () =>
                                _scaffoldKeyJob.currentState?.openDrawer(),
                          ),
                        ),
                                  ...
                                  children: [
                                    ...
                                    GetBuilder<PlanController>(
                                      builder: (pController) {
                                        ...
                                        final imageUrls = pController
                                            .editUploadImage1
                                            .map((clinic) => clinic.url ?? "")
                                            .where((url) => url.isNotEmpty)
                                            .toList();
                                        return ClinicImageCarousel(
                                          imageUrls: imageUrls,
                                        );
                                      },
                                    ),
                                            ...
                                            Expanded(
                                              child: TextField(
                                                ...
                                                onSubmitted: (value) async {
                                                  await jobController
                                                      .getJobListJobSeekers(
                                                        search: value,
                                                        context: context,
                                                      );
                                                  Get.toNamed(
                                                    '/jobListJobSeekersWebPage',
                                                  );
                                                },
                                              ),
                                            ),
                                    ...
                                    GridView.builder(
                                      ...
                                      itemCount:
                                          controller.jobListJobSeekers.length >
                                              6
                                          ? 6
                                          : controller.jobListJobSeekers.length,
                                      itemBuilder: (context, index) {
                                        final job =
                                            controller.jobListJobSeekers[index];
                                        final logoUrl =
                                            (job.logoImage != null &&
                                                job.logoImage!.isNotEmpty)
                                            ? job.logoImage!.first
                                            : null;
